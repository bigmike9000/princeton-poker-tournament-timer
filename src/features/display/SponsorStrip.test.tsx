...
describe('SponsorStrip', () => {
  ...
  it('enlarges only the main sponsor rack at full and short projector heights', () => {
    ...
    expect(displayRule).toMatch(/width:\s*12rem/)
    expect(displayRule).toMatch(/height:\s*3rem/)
    expect(shortHeightRule).toMatch(/width:\s*10\.5rem/)
    expect(shortHeightRule).toMatch(/height:\s*2\.7rem/)
    expect(phoneRule).toMatch(/width:\s*5\.5rem/)
    expect(phoneRule).toMatch(/height:\s*1\.5rem/)
    expect(infoRule).toMatch(/width:\s*clamp\(9rem,\s*14vw,\s*12\.5rem\)/)
    expect(infoRule).toMatch(/height:\s*clamp\(2\.4rem,\s*4vw,\s*3\.3rem\)/)
