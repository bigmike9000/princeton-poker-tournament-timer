...
describe('InfoOverlay', () => {
  ...
  it('keeps the fixed projector layout when only hidden chip lines exceed the raw editor budget', () => {
    ...
    expect(within(chips as HTMLElement).getByRole('group', { name: 'Start with 10 white 1-value chips' })).toBeVisible()
  ...
  it('shows canonical chip cards and live tournament totals', () => {
    ...
    const one = overlay.getByRole('group', { name: 'Start with 10 white 1-value chips' })
    expect(within(one).getByText('1')).toBeVisible()
    expect(within(one).getByText('White')).toBeVisible()
    expect(within(one).getByText('Start with 10')).toBeVisible()

    const five = overlay.getByRole('group', { name: 'Start with 8 red 5-value chips' })
    expect(within(five).getByText('5')).toBeVisible()
    expect(within(five).getByText('Red')).toBeVisible()
    expect(within(five).getByText('Start with 8')).toBeVisible()

    const twentyFive = overlay.getByRole('group', { name: 'Start with 6 green 25-value chips' })
    expect(within(twentyFive).getByText('25')).toBeVisible()
    expect(within(twentyFive).getByText('Green')).toBeVisible()
    expect(within(twentyFive).getByText('Start with 6')).toBeVisible()
